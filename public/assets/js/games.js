document.addEventListener('DOMContentLoaded', function () {
    ...
    function loadGames(games, startIndex = 0, chunkSize = 10) {
        const endIndex = Math.min(startIndex + chunkSize, games.length);
        const chunk = games.slice(startIndex, endIndex);
        ...
        if (endIndex < games.length) {
            requestIdleCallback(() => {
                loadGames(games, endIndex, chunkSize);
            });
        }
    }
